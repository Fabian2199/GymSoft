<?php
  ...
  function get_datos($id_persona){
    ...
    WHERE f.fecha_nac = (SELECT MAX(f.fecha_nac) FROM facturas f JOIN usuarios u ON f.id_cliente = u.id_user JOIN personas p ON u.id_persona = p.id_persona WHERE p.id_persona =".$id_persona.")";
